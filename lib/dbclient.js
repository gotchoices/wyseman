...
// -----------------------------------------------------------------------------
  newClient(discon = true) {				//Get a fresh client for this connection
    ...
    if (this.config.listen) {							//If configured to listen for asynchronous DB events
      this.log.debug("Listening on DB channel:", this.config.listen);
      this.queryQue.push(["listen " + Format.ident(this.config.listen)])	//Register with the DB
