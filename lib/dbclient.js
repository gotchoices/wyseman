...
this.log.trace('dbClient listen:', this.config.listen, typeof(this.config.listen))
...
//let schemaDir = Path.dirname(this.config.schema)
//let loaderFile = Path.join(schemaDir, 'loader.sql')		;this.log.debug('Loader s:', schemaDir, 'l:', loaderFile)
//Fs.writeFileSync(loaderFile, schemaData)
...
// -------------------------------------------------------------------
  query(...args) {		// Attempt a DB query with callback.
    this.log.trace("Query:", this.config.user, args[0].substr(0,128), args[1])
