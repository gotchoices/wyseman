...
module.exports = class Handler {
  constructor({db, control, actions, dispatch, dConfig, log}) {
    ...
    LangCache.request((language, view) => {		//If a report needs language data
this.log.debug('Lang cache view request:', language, view)
//console.log('Lang cache view request:', language, view)
      this.handle({id:'internal', action: 'lang', language, view}, () => {})
    })
  }
...
this.log.trace(" query:", query, "parms:", parms, "tuples:", tuples, "Err:", err)
      ...
      if (res && res.rows) {
        if (tuples == 1) {
          msg.data = res.rows[0]
          if (table == 'wm.table_lang') {
//this.log.debug('LR:', view, msg.language, msg.data)
