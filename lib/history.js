...
// -----------------------------------------------------------------------------
  pushDB(history = this.hist) {
    let sql = ''
//console.log("Hist push:", history.prev)
    ...
    history.prev.forEach((el,ix) => {
      let {obj_typ, obj_nam, obj_ver, module, min_rel, max_rel, deps, col_data, grants, crt_sql, drp_sql} = el
//console.log(" el:", ix, el)
      sql += Format('insert into wm.objects (obj_typ,obj_nam,obj_ver,module,min_rel,max_rel,deps,col_data,grants,crt_sql,drp_sql) values (%L,%L,%s,%L,%s,%s,array[%L]::text[],array[%L]::text[],array[%L]::text[],%L,%L);\n',
        obj_typ, obj_nam, obj_ver, module, min_rel, max_rel, deps, col_data, grants, crt_sql, drp_sql
      )
    })
