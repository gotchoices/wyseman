...
module.exports = class Wyseman {
  constructor(conf, clientPort, userControl) {
    ...
    wss.on('connection', (ws) => {			//When connection from view is open
      let ctx = {db:null, control:null, userControl}
      ctx.db = new DbClient(conf, (channel, message, mine) => {
      ...
      this.log = conf.logger || ctx.db.log
      this.log.trace("In Wyseman constructor conf:", JSON.stringify(conf), " Client port:", clientPort)
//      ctx.control = (userControl ? new userControl() : null)	//will do later
...
this.log.trace("  tab:", tab, " argtypes:", argtypes)
    ...
    switch (action) {
      ...
      case 'delete':
        ...
        tuples = 0;						break;
      default:
      if (!ctx.control && ctx.userControl) ctx.control = new ctx.userControl()		//Start a controller just in time
