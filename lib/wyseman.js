...
//- Errors, like no fields in insert, should return error packet to frontend, not just log
//- 
...
// -------------------------------------------------------------------
  query(...args) {
this.log.debug("Query:", args[0], args[1]);
...
//this.log.debug("  queueing it: ", this.queryQue.length);
    ...
    this.client.query(qstring, parms, (err, res) => {		//Run the user's query
      ...
      if (err) {
        ...
        msg.error = this.error("unexpected rows: " + res.rowCount, "badTuples")
      }
      if (res && res.rows) {
...
// -------------------------------------------------------------------
  handler(msg, sender) {
     ...
     if (!view) return
     if (action == 'lang') { action = 'tuple'; Object.assign(msg, {
       fields: ['title','help','columns','messages'], table: 'wm.table_lang',
       where: {obj: view, language: msg.language || 'en'}
     })
     } else if (msg.action == 'meta') { action = 'tuple'; Object.assign(msg, {
       fields: ['obj','pkey','cols','columns'], table: 'wm.table_meta',
       where: {obj: view}
     })}

     let {table, fields, where, order} = msg
     let [sch, tab] = (table || view).split('.')		//Fixme: need table_split function?
