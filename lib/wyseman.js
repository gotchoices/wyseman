...
module.exports = class Wyseman {
  constructor(conf, clientPort, userControl) {
    ...
    this.db = new dbClient(this.config = conf, (chan, message, mine) => {
      let msg = JSON.parse(message)
//      this.log.debug("Async notify from DB:", chan, msg, mine)
      if (this.ws && !mine) this.ws.send(JSON.stringify({action: 'notify', data:msg}), err => {
...
this.log.trace("BuildSelect", fields, table, where, order)
    let wh = '', ord = ''
