...
const Fields = 'obj_typ, obj_nam, obj_ver, module, min_rel, max_rel, deps, grants, col_data, crt_sql, drp_sql'
...
//console.log("Schema:", this.release, dbh, arch)
      ...
      this.boot = this.bootSql(db, this.release) || this.decode(arch.boot, true),
      this.dict = this.dictSql(db) || this.decode(arch.dict, true),
      this.init = init || this.decode(arch.init, true),		//Initialization comes from wmi files or history
...
// -----------------------------------------------------------------------------
  get(compress = true, doHash = true) {
    ...
    this.objects.forEach(el => {
      let { obj_typ, obj_nam, obj_ver, module, min_rel, max_rel, deps, grants, col_data} = el
      ...
      objects.push({obj_typ, obj_nam, obj_ver, module, min_rel, max_rel, deps, grants, col_data, create, drop})
    })
    if (doHash) {
...
--      insert into wm.objects (obj_typ,obj_nam,obj_ver,module,min_rel,max_rel,deps,col_data,grants,crt_sql,drp_sql)
--        values (j->>'obj_typ', j->>'obj_nam', (j->'obj_ver')::int, j->>'module',
