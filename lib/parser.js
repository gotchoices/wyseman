...
module.exports = class {
  constructor(db) {
    ...
    this.tcl = new Tcl()
    this.fileName = ''
    this.dirNames = []			//Remember what directories we've been in
...
//console.log("Check runtime:")
  ...
  paths() {			//Return directories we parsed
...
//console.log('file:', file, "ext:", Path.extname(file), 'res:', Path.resolve(file))
    ...
    this.fileName = Path.basename(file)			//Pass around the side to sql generator
...
//console.log('sql:', sql)
    ...
    if (!this.dirNames.includes(dirName)) this.dirNames.push(dirName)
    try {
