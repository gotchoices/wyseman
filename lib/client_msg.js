...
this.debug('Server open:', !!sender)
  ...
  async onMessage(msg) {				//Call with packets from the server
...
this.debug('From server: ', pkt, id, action, error)
...
//this.debug("To localStorage:", index)
      await this.localStore.set(index, data)			//Save also to browser cache
...
//this.debug('langDefs:', langObj)
  ...
  async request(id, action, opt, cb) {		//Ask to receive specified CRUD+ information back asynchronously
    if (typeof opt === 'string') {opt = {view: opt}}	//Shortcut: can supply view rather than full options object
this.debug("Request ID:", id, "Act:", action, "Opt:", JSON.stringify(opt).slice(0,128), !!this.sender)
...
//this.debug("From localStorage?:", idx, data)
          let localVal = await this.localStore.get(idx)
