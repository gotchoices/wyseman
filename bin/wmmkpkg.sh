...
if [ -f init.tcl ]; then
    init="[list source [file join \$dir init.tcl]]\n"
else
    init=""
fi
...
package ifneeded $libname $version [list set ${libname}_library \$dir]\n$init[list tclPkgSetup \$dir $libname $version {
EOF
